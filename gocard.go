...
func (cc *CardNumber) First6() string {
	return *cc[0:6]
}
...
func (cc *CardNumber) SaltMD5(salt string) []byte {
	h := md5.New()
	io.WriteString(h, *cc+salt)
	return h.Sum(nil)
}

func (cc *CardNumber) SaltSHA1(salt string) []byte {
	h := sha1.New()
	io.WriteString(h, *cc+salt)
	return h.Sum(nil)
}

func (cc *CardNumber) SaltSHA256(salt string) []byte {
	h := sha256.New()
	io.WriteString(h, *cc+salt)
	return h.Sum(nil)
}

func (cc *CardNumber) SaltSHA512(salt string) []byte {
	h := sha512.New()
	io.WriteString(h, *cc+salt)
	return h.Sum(nil)
}
